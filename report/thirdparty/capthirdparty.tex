...
The Ticketmaster query encoding test asserts that query parameters such as spaces are encoded exactly once. The provider lets \texttt{RestClient}'s URI builder encode the query values, preventing \texttt{\%2520} double-encoding.

\section{DiscoveryService Aggregation}

The discovery level provider aggregation behavior is tested separately in \texttt{DiscoveryServiceTest}. This class uses small in memory \texttt{EventProvider} implementations to focus on aggregation rules without HTTP setup. It verifies that blank queries return no results, unconfigured providers are skipped, duplicate URLs are removed, and results are sorted by start time.

The aggregation test verifies that an outage in one configured provider does not prevent results from other providers. \texttt{DiscoveryService} catches provider failures, skips the failed provider, and continues merging healthy provider results.
