...
This document is written as a walkthrough of the solution. Chapter~\ref{cap:sut} first explains the system under test and the implementation corrections. Chapter~\ref{cap:strategy} maps each assignment requirement to the implemented evidence. The following chapters then walk through each testing level, explaining what part of the code is exercised, why that testing level was chosen, what data or mocks are used, and which defects are detected and guarded by regression tests.
...
\item Use automated tests to expose implementation bugs and keep those scenarios as regression coverage.
\item Document the production corrections made for each detected defect.
\end{itemize}

The default Maven run executes the complete suite of 61 tests and passes.
The automated tests detected implementation defects, and those defects were corrected in the production code. The detected bugs and their corresponding corrections are documented in Chapter~\ref{cap:bugs}.
