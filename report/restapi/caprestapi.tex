...
Additional direct controller tests in \texttt{DiscoveryControllerTest} and \texttt{MeetingControllerTest} complement the integration tests by exercising branch-heavy controller decisions without depending on external discovery providers or a full Spring context.
...
Two REST tests cover corrected invite response error handling. \texttt{respondAcceptsOnlyKnownActions} expects unknown actions such as \texttt{maybe} to return \texttt{400 Bad Request}. \texttt{userCannotRespondToSomeoneElsesInvite} expects a clean \texttt{404 Not Found} response when the authenticated user has no invite for that meeting. Both behaviors are implemented through explicit controller side HTTP status mapping.
